...
/// 运行数据
/// </summary>
public class RuntimeDataManager : IDisposable
{
    public MissionManager modMission =new MissionManager();

    public int CurrBattleID = 1;
    ...
    {
        this.Team.Add(GameEntry.Role.GetRoleInfo(roleID));
    }
    ...
    /// 自动接收任务
    /// </summary>
    public void AutoAcceptMisson()
    {
        List<MissionEntity> missionList = GameEntry.DataTable.DataTableManager.MissionDBModel.GetList();
        for (int i = 0; i < missionList.Count; i++)
        {
            MissionEntity missionInfo = missionList[i];
            if (missionInfo != null)
            {
